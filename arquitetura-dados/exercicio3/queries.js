// 1. 
var player_ids = db.medias.find({},{player_id:1}).sort({G_mean:-1}).limit(10).map((media)=>media['player_id']);
...
// 3.
db.medias.find({player_position:"gol"}).sort({DD_mean:-1}).limit(1).pretty();
